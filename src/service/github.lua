...
local config = ...
local source_config = config.sources.github
...
local function publish(patch)
	ltask.send(state_service, "merge", "github", patch)
end
...
local function fail(message, headers, now, now_centiseconds)
	...
	publish(remote.failure_patch(
		...
		delay,
		now))
	ltask.log.error("GitHub refresh failed", message, "retry_ms", delay)
end

local function request_headers()
	local token = os.getenv(source_config.token_env)
	if not token or token == "" then
		return nil, ("environment variable %s is not set"):format(
			source_config.token_env)
	end
	local headers = {
...
local function refresh(now, now_centiseconds)
	local headers, header_error = request_headers()
	if not headers then
		fail(header_error, nil, now, now_centiseconds)
		return
	end
	...
	if response.status == 304 then
		if last_success_at == 0 then
			fail(
				"GitHub returned 304 before an initial snapshot",
				response.headers,
				now,
				now_centiseconds)
			return
		...
		failures = 0
		schedule(delay, now_centiseconds)
		publish(remote.success_metadata(delay, now))
		return
	end
	if response.status < 200 or response.status >= 300 then
		fail(
			("GitHub HTTP status %d"):format(response.status),
			response.headers,
			now,
			now_centiseconds)
		return
	end
	local parsed, notifications = pcall(
		remote.parse_github,
		response.body,
		source_config.max_items)
	if not parsed then
		fail(notifications, response.headers, now, now_centiseconds)
		return
	end
	...
	}, remote.success_metadata(delay, now)))
end
...
ltask.idle_handler(function()
	...
	if now_centiseconds < deadline then
		...
		if wait_error then
			ltask.log.error("GitHub wait failed", wait_error)
...
function S.reload(new_config)
	local old_url = source_config.url
	local old_token_env = source_config.token_env
	source_config = new_config.sources.github
	if source_config.url ~= old_url or
		source_config.token_env ~= old_token_env then
		etag = nil
	end
	local _, now_centiseconds = ltask.now()
	deadline = now_centiseconds
